...
- Supports individual Agent interview and batch interview
...
# This fixes the issue where the OASIS third-party library reads files without specifying an encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes the issue where third-party libraries (e.g., OASIS) read files without specifying an encoding
        """
...
# Script is always located in the backend/scripts/ directory
...
# Add the filter immediately when the module loads, ensuring it takes effect before camel code runs
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize the simulation log configuration
    
    Args:
        simulation_dir: Simulation directory path
    """
    # Disable OASIS verbose logs
...
# Available Twitter actions (INTERVIEW not included; INTERVIEW can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (INTERVIEW not included; INTERVIEW can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing the result, or a dictionary containing an error
        """
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": interview Twitter platform only
                - "reddit": interview Reddit platform only
                - None/not specified: each Agent interviews both platforms simultaneously
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Agents that need to interview both platforms
        ...
        # Handle interviews for the Twitter platform
        ...
        # Handle interviews for the Reddit platform
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows the real entity names to be displayed in actions.jsonl, rather than codes like "Agent_0"
    
    Args:
        config: the content of simulation_config.json
        
    Returns:
        mapping dictionary from agent_id -> entity_name
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and supplement with full context information
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value from the last read (using rowid instead of created_at, since created_at formats differ across platforms)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: list of actions, each element containing agent_id, agent_name, action_type, action_args (with context info)
    ...
    try:
        ...
        # This avoids the created_at format difference issue (Twitter uses an integer, Reddit uses a datetime string)
        cursor.execute("""
...
def _enrich_action_context(
    ...
    """
    Supplement context information for action (post content, username, etc.)
    ...
    try:
        ...
        # Quote post: supplement original post content, author, and quoted comment
            ...
            # Get the comment content of the quoted post (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Follow user: supplement the name of the followed user
        ...
        # Mute user: supplement the name of the muted user
        ...
        # Create comment: supplement the post information being commented on
        ...
        # Failure to supplement context does not affect the main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for accelerating parallel simulation:
    ...
    If a boost LLM is configured, different platforms can use different API providers during parallel simulation, improving concurrency.
...
def get_active_agents_for_round(
    ...
    """Determine which Agents to activate in this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        action_logger: PlatformActionLogger
        main_logger: SimulationLogManager
    ...
    # Get the real Agent name mapping from the config file (using entity_name instead of the default Agent_X)
    ...
    result.env = oasis.make(
        ...
        semaphore=30,  # Limit the maximum concurrent LLM requests to prevent API overload
    )
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row number in the database (using rowid to avoid created_at format differences)
    ...
    # If maximum rounds are specified, truncate
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Also log round end when there are no active agents (actions_count=0)
            if action_logger:
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (if available; otherwise falls back to the general configuration)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation, without entering command-waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create the shutdown event at the start of the main function to ensure the entire program can respond to shutdown signals
    ...
    # Initialize log configuration (disable OASIS logs, clean up old files)
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep, to be able to respond to shutdown_event
                try:
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: does not exit after simulation completes, waits for interview commands
    When a termination signal is received, the following steps are needed:
    1. Notify the asyncio loop to exit waiting
    2. Allow the program to properly clean up resources (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (allowing the loop to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (to prevent warnings on exit)
        try:
