...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep knowledge graph into the Agent Profile required for OASIS simulation
    
    Optimization features:
    1. Invoke Zep graph retrieval to obtain richer context
    ...
    # Group/institution entity types (require generating representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to obtain rich information related to an entity
        
        Zep has no built-in hybrid search interface; edges and nodes must be searched separately and results merged.
        Parallel requests are used to search simultaneously for efficiency.
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if the entity is a group/institution type"""
    ...
    def _generate_profile_with_llm(
        ...
        Differentiate based on entity type:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Check if the output was truncated (finish_reason is not 'stop')
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First attempt to fix the truncated case
        content = self._fix_truncated_json(content)
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, restoring existing reality to the greatest extent possible.
...
2. persona: Detailed persona description (2000-character plain text), including:
   ...
   - Personal background (key experiences, connections to events, social relationships)
   ...
   - Stance and opinions (attitude toward topics, content that may provoke or move them)
   - Distinctive traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona; describe this individual's connection to events and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for an institution/group entity, restoring existing reality to the greatest extent possible.
...
2. persona: Detailed account profile description (2000-character plain text), including:
   - Institution basic information (official name, nature of the institution, founding background, primary functions)
   ...
   - Institutional memory (important part of the institutional persona; describe this institution's connection to events and its existing actions and reactions in those events)
...
- Institutional account speech must match its identity and positioning"""
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Output only to console (avoid duplication; logger no longer outputs full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to a file (choosing the correct format based on platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles as CSV format (conforming to official OASIS requirements)
        ...
        - user_char: Used internally, LLM system prompt, determines how the Agent thinks and acts
        - description: Displayed externally, bio visible to other users
        """
        import csv
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # Required OASIS fields - ensure all have default values
