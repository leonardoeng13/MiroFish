...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Set child process environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues where third-party libraries (e.g. OASIS) read files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support, makes all open() calls use UTF-8 by default
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to the simulation directory (database files etc. will be created here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure all related processes are terminated when the server shuts down
            )
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines if a platform is enabled by checking whether its corresponding actions.jsonl file exists
        ...
        # Check which platforms are enabled (determined by whether files exist)
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                # Fall back to directly terminating the process
                try:
                    process.terminate()
                    process.wait(timeout=5)
                except Exception:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: use the platform in the record first, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform is automatically set to twitter based on file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (platform is automatically set to reddit based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used to force a simulation restart)
        ...
        Note: Does not delete configuration files (simulation_config.json) or profile files
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean up (avoid printing useless logs when there are no processes)
    ...
    def register_cleanup(cls):
        ...
        Called when the Flask application starts to ensure all simulation processes are cleaned up when the server shuts down
        """
        ...
        # If not in debug mode, this env var won't be set, and registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, only register in the reloader child process; always register in non-debug mode
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered, prevent child processes from trying again
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Call the original signal handler to allow Flask to exit normally
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        """
        Check if the simulation environment is alive (able to receive Interview commands)

        Args:
            simulation_id: Simulation ID

        Returns:
            True if environment is alive, False if environment has been closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms simultaneously and return combined result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default to only interviewing the Twitter platform
                - "reddit": Default to only interviewing the Reddit platform
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation, causing it to exit the command-waiting mode gracefully
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specify Agent ID (optional, only get history for this Agent)
