...
# Standard daily schedule configuration
...
class TimeSimulationConfig:
    """Time simulation configuration (based on standard daily schedule habits)"""
    # Total simulation duration (simulation hours)
    total_simulation_hours: int = 72  # Default: simulate 72 hours (3 days)
    
    # Time represented per round (simulation minutes) - default 60 minutes (1 hour), speeding up time flow
    ...
    # Peak hours (evening 19:00-22:00, typically the most active period)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Off-peak hours (early morning 0:00-5:00, almost no activity)
    off_peak_hours: List[int] = field(default_factory=lambda: [0, 1, 2, 3, 4, 5])
    off_peak_activity_multiplier: float = 0.05  # Extremely low activity in the early morning hours
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions required to trigger virality)
    viral_threshold: int = 10
    
    # Echo chamber effect strength (degree of clustering of similar viewpoints)
...
class SimulationConfigGenerator:
    ...
    Uses LLM to analyze simulation requirements, document content, and graph
    entity information to automatically generate optimal simulation parameter
    configurations.
    ...
    AGENT_SUMMARY_LENGTH = 300           # Entity summary within Agent configuration
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # max_tokens is not set, allowing the LLM to output freely
                )
...
- Adapt the schedule to the local timezone and audience habits described in the simulation requirements
- Almost no activity from 0:00-5:00 AM (activity multiplier 0.05)
- Gradually becoming active from 6:00-8:00 AM (activity multiplier 0.4)
- Moderate activity during working hours 9:00-18:00 (activity multiplier 0.7)
- Peak period from 19:00-22:00 evening (activity multiplier 1.5)
- Activity declines after 23:00 (activity multiplier 0.5)
- General pattern: low in early morning, increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only; adjust specific time periods based on event nature and group characteristics
  - Example: student peak may be 21:00-23:00; media is active all day; official institutions only during working hours
  - Example: a sudden trending topic may cause late-night discussion, so off_peak_hours can be shortened
...
- off_peak_hours (int array): Off-peak hours, usually early morning
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration must reflect the schedule habits appropriate for the simulated audience and timezone."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time configuration (standard daily schedule)"""
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,  # 1 hour per round, accelerating time flow
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result and validate that agents_per_hour values do not exceed total agent count"""
        # Get raw values
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no activity in the early morning hours
    ...
    def _generate_event_config(
        ...
        # Get list of available entity types for the LLM to reference
...
**Important**: poster_type must be selected from the "Available Entity Types" above, so that initial posts can be assigned to a suitable Agent for publishing.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Matches the most suitable agent_id for each post based on its poster_type
        """
...
- **Times must reflect realistic schedule habits for the simulated audience**: almost no activity from 0:00-5:00 AM, most active from 19:00-22:00 PM
- **Official institutions** (University/GovernmentAgency): low activity (0.1-0.3), active during working hours (9:00-17:00), slow response (60-240 minutes), high influence (2.5-3.0)
- **Media** (MediaOutlet): moderate activity (0.4-0.6), active all day (8:00-23:00), fast response (5-30 minutes), high influence (2.0-2.5)
- **Individuals** (Student/Person/Alumni): high activity (0.6-0.9), mainly active in the evening (18:00-23:00), fast response (1-15 minutes), low influence (0.8-1.2)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, based on audience schedule habits>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations must reflect realistic schedule habits for the simulated audience."
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM did not generate a config, use rule-based generation
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration based on rules (standard daily schedule)"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: active during work hours and evening, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
