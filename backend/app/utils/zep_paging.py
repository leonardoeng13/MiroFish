...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page with exponential-backoff retry on transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch all graph nodes with pagination, returning at most max_items (default 2000). Each page request includes retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, returning the complete list. Each page request includes retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
