...
# Adding this prefix prevents Agents from calling tools and prompts them to reply with plain text
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, respond to me directly in text without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize an interview prompt by adding a prefix to prevent Agent tool calls
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // optional, retrieved from project if not provided
...
def prepare_simulation():
    """
    Prepare the simulation environment (async task, all parameters generated intelligently by LLM)
    ...
    Features:
    - Automatically detects completed preparation to avoid redundant generation
    - Returns existing results if preparation is already complete
    ...
    4. Intelligently generate simulation configuration via LLM (with retry)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started | Existing preparation found",
    ...
    try:
        ...
        # Check whether preparation is already complete (avoid redundant generation)
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing preparation found; no need to regenerate",
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Quick entity read (no edge information needed, just count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Skip edge information for speed
            )
            # Save entity count to state (available to frontend immediately)
...
def get_prepare_status():
    ...
    1. Query an ongoing task's progress via task_id
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing preparation found",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation has not started yet; please call /api/simulation/prepare",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # Task not found; if simulation_id is provided, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterate through the reports directory, find the report(s) matching simulation_id,
    and return the most recent one (sorted by created_at).
    ...
    try:
        ...
        # Sort by creation time descending, return most recent
...
def get_simulation_history():
    ...
    Used for the homepage history display; returns a rich simulation list
    including project name, description, and other information
            ...
            "data": [
                {
                    "simulation_id": "sim_xxxx",
                    "project_id": "proj_xxxx",
                    "project_name": "Campus Sentiment Analysis",
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds; fall back to recommended rounds if not set
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profiles in real time (for viewing progress during generation)
    ...
    - Returns additional metadata (file modification time, whether generation is in progress, etc.)
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (may be in the process of being written): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (file modification time, whether generation is in progress, etc.)
    - Can return partial information even if the config has not finished generating
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (may be in the process of being written): {e}")
                config = None
        ...
        # If config exists, extract key summary statistics
        if config:
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (full config intelligently generated by LLM)
    
    Returns:
        - time_config: Time configuration (simulation duration, rounds, peak/off-peak periods)
        - agent_configs: Activity configuration per Agent (activity level, posting frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download a simulation run script (general scripts located in backend/scripts/)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: maximum simulation rounds, used to cap long simulations
    ...
    About the force parameter:
        - When enabled, if a simulation is running or completed, it is stopped and run logs are cleared
        ...
        - Useful when a simulation needs to be rerun

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) during simulation
          are updated to the Zep graph in real time
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete; check whether a simulation process is running
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running; please call /stop first, or use force=true to force a restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for the frontend to display real-time activity
    ...
    try:
        ...
        # recent_actions shows only both platforms' content for the latest round
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used by the frontend to display progress bar and timeline views
...
def interview_agent():
    ...
    (the simulation enters command-waiting mode after completing its simulation loop)
    ...
    Returns (platform not specified, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // no platform specified; uses default
                }
            ],
    ...
    try:
        ...
        # Optimize each interview item's prompt to prevent Agent tool calls
...
def interview_all_agents():
    ...
    try:
        ...
        # Optimize prompt to prevent Agent tool calls
        optimized_prompt = optimize_interview_prompt(prompt)
...
def get_env_status():
    ...
    Check whether the simulation environment is alive (capable of receiving Interview commands)
...
def close_simulation_env():
    ...
    Send a close-environment command to the simulation, causing it to gracefully exit
